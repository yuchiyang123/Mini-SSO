...
- JWT 撤銷機制（存 Redis）：登出會讓那個 token 真的失效，不是只清 Cookie
- Refresh token（存 Redis）：access token 過期後不用重新登入，`POST /api/auth/refresh` 換一組新的，且每次使用都會 rotate（舊的立刻作廢）
...
- Docker Compose 一鍵部署（nginx + API + SQL Server + Redis），全部服務都有 healthcheck
...
- Redis（撤銷的 access token 黑名單 + refresh token 存放，透過 `IDistributedCache`）
...
```
Mini-SSO/
├── Controllers/AuthController.cs      # 登入/註冊/登出/refresh/第三方登入/CSRF/me 端點
├── Services/
│   ├── AuthService.cs                 # 業務邏輯（含 ExternalLoginAsync、登入鎖定、refresh token）
│   └── TokenStore.cs                  # Redis 存取層：撤銷的 access token 黑名單 + refresh token
├── Model/
...
├── Program.cs                         # 組合根：JWT / OAuth / CORS / CSRF / Redis / 健康檢查 / DB / 安全標頭
└── Dockerfile
Mini-SSO.Tests/
...
以下兩種資料**不放 SQL Server，放 Redis**（見 `Services/TokenStore.cs`），因為本質上是有 TTL、用完即丟的暫存資料，用 Redis 的原生過期機制比在關聯式資料庫裡存一張表、還要另外寫背景工作清理來得直接：
- **撤銷的 access token 黑名單**：key 是 `mini-sso:revoked:{jti}`，TTL = 這個 token 原本的到期時間。
- **Refresh token**：key 是 `mini-sso:refresh:{token}`，value 是對應的 `UserId`，TTL = `Jwt:RefreshExpireDays`。
...
- 登出時（`AuthController.Logout`），從目前的 JWT claims 取出 `jti`（token 的唯一 ID）和 `exp`，呼叫 `AuthService.RevokeTokenAsync` → `TokenStore.RevokeAccessTokenAsync` 寫進 Redis，TTL 設成這個 token 原本還剩多久到期。
- `Program.cs` 的 `AddJwtBearer` 加了 `OnTokenValidated` 事件：每次驗證 JWT 時，額外查一次這個 `jti` 是否在 Redis 黑名單裡，有的話 `context.Fail(...)`，直接視為驗證失敗（`401`）。
- 不需要額外的背景清理工作——Redis 的 TTL 到了會自己清掉這個 key。

### Refresh Token（access token 過期後不用重新登入）

帳密登入、SSO 登入成功後，除了原本的 `token`（access token，短效）Cookie，現在還會多發一個 `refresh_token` Cookie（`HttpOnly`，`Path=/api/auth`，限縮曝露面，存活天數由 `Jwt:RefreshExpireDays` 決定，預設 30 天）。

```
POST /api/auth/refresh
```
- 從 `refresh_token` Cookie 讀值，去 Redis 查對應的 `UserId`（`TokenStore.GetUserIdForRefreshTokenAsync`）。
- 找不到（沒有/過期/已用過）→ `401`。
- 找到 → **rotate**：立刻撤銷這個 refresh token（`TokenStore.RevokeRefreshTokenAsync`），簽發一組全新的 access token + refresh token，一起用新 Cookie 蓋掉舊的。

Rotate 的用意：refresh token 只能用一次，用過就換下一個。如果同一個 refresh token 被用了兩次（例如被偷走的人跟正常使用者都在用），第二次一定會拿到 `401`，可以當作偵測「token 可能外洩」的信號。

登出時（`Logout`）也會一併撤銷當下的 refresh token，並清掉 `refresh_token` Cookie，避免登出後還能用舊的 refresh token 換到新的 access token。
...
> **踩過的坑 2**：`nginx.conf` 原本用 `upstream { server api:8080; }` 搭配 `proxy_pass http://mini_sso_api;`，這種寫法 nginx 只會在**啟動當下**把 `api` 這個服務名稱解析成 IP 並快取住。開發時常常會單獨 `docker compose up -d` 重建 `api` 容器（例如改完 `.env` 之後），容器會換一個新的內部 IP，但 nginx 沒有重啟就還在打舊 IP，變成 `502 Bad Gateway`（nginx log 會看到 `connect() failed (111: Connection refused)`）。修法是用 `resolver 127.0.0.11 valid=10s;`（Docker 內建 DNS）搭配 `set $api_upstream http://api:8080; proxy_pass $api_upstream;`（用變數而不是寫死的 upstream block），讓 nginx 每次請求都重新解析，不會卡住舊 IP。**之後只要重建了 `api` 容器，記得順便確認一下 nginx 是否還連得上**（如果改的是 nginx.conf 本身則一定要 `docker compose restart nginx`）。
...
| POST | `/api/auth` | 帳密登入（同 IP 失敗 5 次鎖 10 分鐘），成功會發 access + refresh token | 否 | 是 |
| POST | `/api/auth/refresh` | 用 refresh token 換一組新的 access + refresh token（rotate） | 否（靠 refresh_token Cookie） | 是 |
| POST | `/api/auth/create` | 註冊新帳號 | 否 | 是 |
| GET | `/api/auth/valid/username?username=` | 查詢帳號是否可用 | 否 | - |
| POST | `/api/auth/logout` | 登出（撤銷目前的 access token + refresh token，不是只清 Cookie） | 是 | 是 |
...
| `Redis__ConnectionString` | `Redis:ConnectionString` | Redis 連線字串（撤銷黑名單 + refresh token 存放），docker-compose 內固定是 `redis:6379` |
| `Jwt__Key` / `Jwt__Issuer` / `Jwt__Audience` / `Jwt__ExpireMinutes` | `Jwt:*` | JWT（access token）簽章金鑰與有效期 |
| `Jwt__RefreshExpireDays` | `Jwt:RefreshExpireDays` | Refresh token 有效天數，預設 30 |
...
需要本機或可連線的 SQL Server（`appsettings.Development.json` 或 user-secrets 設定連線字串），以及一個 Redis（預設抓 `appsettings.json` 的 `localhost:6379`，本機開發最簡單就是 `docker run -p 6379:6379 redis:7-alpine` 開一個）。啟動時會自動執行 EF Core migration 並在資料庫是空的時候建立預設帳號 `admin` / `admin123`。
...
`docker-compose.yml` 會啟動四個服務：
- `db`：SQL Server 2022，資料存在 named volume `mssql-data`，有 healthcheck
- `redis`：Redis 7，資料存在 named volume `redis-data`，有 healthcheck
- `api`：本專案，等 `db`、`redis` 都 healthy 後才啟動，啟動時自動跑 migration + seed；**不對外開 port**
...
- [x] Refresh token：登入後 `/refresh` 可以換到新的 access+refresh token；換過的舊 refresh token 再拿去用會 `401`；登出後舊 refresh token 也會 `401`
- [x] Redis：`TTL` 指令確認撤銷的 access token 黑名單 key 有正確設定到期時間，跟 access token 本身的到期時間一致
- [x] nginx DNS 解析：重建 `api` 容器（換新 IP）後，套用 `resolver` + 變數寫法的 `nginx.conf` 不需要重啟 nginx 也能正常連線（用來取代舊的 `upstream` 靜態解析寫法）
...
- Refresh token 用完即 rotate（一次性），同一個 refresh token 被重複使用會被視為異常並拒絕，可作為偵測 token 外洩的信號；`refresh_token` Cookie 也限縮 `Path=/api/auth`，減少曝露面。
